...
class ChatSampleState extends State<ChatSample> {
  ...
  Widget _buildMessage(
    ...
    Widget liveStreamCard() {
      // Match notification behavior: invitation messages work exactly like notifications
      // The channelId in the message IS the stream link, just like in notifications
      final hasValidData =
          data.channelId != null &&
          data.channelId!.isNotEmpty &&
          data.sender?.id != null;

      // NOTE: We don't check liveStreamChannels because it's unreliable
      // (often empty even when stream is active). Notifications don't check it either.
      // All invitations are treated as potentially active - if the stream ended,
      // Agora will handle the error when user tries to join.

      return GestureDetector(
        onTap: !isSentByUser && hasValidData
            ? () {
                bool connectionCallBack = false;
                // Use the channelId from the message - this IS the stream link
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Container(
                  decoration: BoxDecoration(
                    // Always show as active (blue) when user has valid data to join
                    color: hasValidData
                ...
                Visibility(
                  visible: !isSentByUser && hasValidData,
                  child: TextView(
                    margin: EdgeInsets.all(10),
                    text: 'Join Video Stream',
