...
import '../../../services/agora/agora_cloud_recording_service.dart';
...
class LiveStreamCtrl extends GetxController {
  ...
  RxInt recordingDuration = RxInt(0); // in seconds
  String? recordedVideoPath;
  String? resourceId; // For Agora cloud recording
  String? sid; // Session ID for Agora cloud recording
  String? _agoraToken; // Store the token for recording
  ...
  Future<void> inviteForLive(ProfileDataModel user) async {
    ...
    AppUtils.log('Is Success: ${result.isSuccess}');
    AppUtils.log('Error: ${result.getError}');

    if (result.isSuccess) {
      invitedUsers.add(user);
      AppUtils.log('✓ Invitation sent successfully to ${user.name}');
      AppUtils.toast("Invitation sent to ${user.name}!");
      return;
    } else {
      final errorMsg = result.getError ?? 'Unknown error occurred';
      AppUtils.log('✗ Failed to send invite: $errorMsg');
      AppUtils.toastError('Failed: $errorMsg');
  ...
  Future<List<ProfileDataModel>> getFriends() async {
    AppUtils.log('getFriends() called - fetching followings...');
    await ProfileCtrl.find.getMyFollowings();
    AppUtils.log('Following list count: ${_friendList.length}');
    AppUtils.log('Live users count: ${liveUsers.length}');
    AppUtils.log('Invited users count: ${invitedUsers.length}');
  ...
  /// Start recording the live stream using Agora Cloud Recording
  Future<void> startRecording() async {
    try {
      ...
      final channelName = streamCtrl.value.channelId ?? '';
      final uid = getNumericUserId().toString();

      AppUtils.log('=== Starting Agora Cloud Recording (Direct API) ===');
      AppUtils.log('Channel: $channelName');
      AppUtils.log('UID: $uid');

      // Get token for recording (if you need a fresh one)
      final tokenData = await ProfileCtrl.find.getUserAgoraToken(
        channelName,
        uid,
        isHost,
      ...
      AppUtils.log('Step 1: Acquiring resource ID...');
      final acquireResult = await AgoraCloudRecordingService.acquire(
      ...
      if (!acquireResult.success) {
        throw Exception(
          acquireResult.errorMessage ?? 'Failed to acquire resource ID',
        );
      }

      resourceId = acquireResult.resourceId;
      AppUtils.log('✓ Resource ID acquired: $resourceId');
      ...
      AppUtils.log('Step 2: Starting recording...');
      final startResult = await AgoraCloudRecordingService.start(
        channelName: channelName,
        uid: uid,
      ...
      sid = startResult.sid;
      AppUtils.log('✓ Recording started - SID: $sid');
      ...
      _recordingTimer = Timer.periodic(Duration(seconds: 1), (timer) {
        ...
        if (recordingDuration.value >= maxRecordingDuration) {
          stopRecording();
          AppUtils.toast(
            'Recording stopped: Maximum duration (10 minutes) reached',
          );
        }
      });

      AppUtils.toast('Cloud recording started');
      AppUtils.log('✓ Cloud recording is now active');
    } catch (e) {
      AppUtils.logEr('Error starting recording: $e');
      AppUtils.toastError('Failed to start recording: ${e.toString()}');
  ...
  Future<void> stopRecording() async {
    try {
      ...
      AppUtils.log('=== Stopping Agora Cloud Recording (Direct API) ===');
      ...
      if (resourceId != null && sid != null) {
        ...
        AppUtils.log('Sending stop request...');
        AppUtils.log('Channel: $channelName');
        AppUtils.log('ResourceId: $resourceId');
        AppUtils.log('SID: $sid');

        final stopResult = await AgoraCloudRecordingService.stop(
          channelName: channelName,
          uid: uid,
          resourceId: resourceId!,
          sid: sid!,
        ...
        if (stopResult.success) {
          ...
          recordedVideoPath = stopResult.fileUrl;

          AppUtils.log('✓ Recording stopped successfully');
          AppUtils.log('File URL: $recordedVideoPath');
          AppUtils.log('Duration: ${recordingDuration.value}s');

          if (recordedVideoPath != null) {
            AppUtils.toast('Recording saved to cloud');
          } else {
            AppUtils.log(
              'Warning: No file URL received, video may need processing time',
            );
            AppUtils.toast('Recording stopped - processing...');
            // Set a placeholder indicating video is being processed
            recordedVideoPath = 'processing';
          }
        } else {
          AppUtils.logEr(
            'Failed to stop recording: ${stopResult.errorMessage}',
          );
          AppUtils.toastError('Recording may not have saved properly');
          recordedVideoPath = null;
        }
      } else {
        AppUtils.logEr('Missing resourceId or sid');
        recordedVideoPath = null;
      }
      ...
      AppUtils.toastError('Error stopping recording');
      recordedVideoPath = null;
      isRecording.value = false;
      _recordingTimer?.cancel();
      _recordingTimer = null;
