...
class PostCard extends StatelessWidget {
  ...
  static const bool useInteractiveViewer = false;
  static const bool usePinchGestureHandler = true;
  static const bool useCustomPageView = false;
  ...
  Widget imageView(
    ...
    final defaultHeight =
        MediaQuery.of(navState.currentContext!).size.height * 0.4;

    return useInteractiveViewer
        ? InteractiveViewer(
            panEnabled: true,
            minScale: 1,
            maxScale: 4,
                        ...
                        : SizedBox(
                            ...
                            child: PageView.builder(
                              ...
                              itemBuilder: (context, index) {
                                final data = imageUrls[index];
                                final height =
                                    data.y ??
                                    MediaQuery.of(context).size.height * 0.6;
                                final width =
                                    data.x ?? MediaQuery.of(context).size.width;
                                final imageUrl = imageUrls[index].file;
                                return imageView(
                                  imageUrl!,
                                  context: context,
                                  height: height,
...
class _PinchGestureHandlerState extends State<_PinchGestureHandler> {
  ...
  // Gesture handler configuration flags
  static const bool useSimplePinchZoom = true;
  static const bool useRawGestureDetector = false;

  @override
  Widget build(BuildContext context) {
    return useSimplePinchZoom
        ? PinchZoom(maxScale: 3.5, child: widget.child)
        : useRawGestureDetector
        ? RawGestureDetector(
            gestures: {
