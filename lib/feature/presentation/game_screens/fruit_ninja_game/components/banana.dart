...
class Banana extends ThrowFruit {
  ...
  Banana(FruitGame game, double x, double y) : super(game) {
    resize(x: x, y: y);
    flyingSprite = [
      Sprite(game.images.fromCache('banana.png')),
      Sprite(game.images.fromCache('banana.png')),
    ];
    deadSprite = Sprite(game.images.fromCache('banana-cut-1.png'));
    deadSprite2 = Sprite(game.images.fromCache('banana-cut-2.png'));
