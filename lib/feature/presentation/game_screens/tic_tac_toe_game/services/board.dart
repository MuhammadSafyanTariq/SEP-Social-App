...
class BoardService {
  ...
  void newMove(int i, int j) {
    ...
    List<List<String>> currentBoard = _board.value.map((e) => List<String>.from(e)).toList();
  ...
  void resetBoard() {
    ...
    _player.add(_start);
    _boardState$.add(const MapEntry(BoardState.Play, null));
    if (_player.value == "O") {
      _player.add("O");
    } else {
      _player.add('X');
  ...
  void _initStreams() {
    ...
    _boardState$ = BehaviorSubject<MapEntry<BoardState, String?>>.seeded(
      const MapEntry(BoardState.Play, null),
    );
