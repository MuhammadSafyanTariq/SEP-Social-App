...
class _EmailLoginFormState extends State<EmailLoginForm> {
  ...
  void _submitForm() async {
    AppUtils.log("🔐 LOGIN FORM SUBMITTED");
    AppUtils.log("📧 Email: ${emailController.text}");
    AppUtils.log("🔑 Password Length: ${passwordController.text.length}");

    if (_formKey.currentState!.validate()) {
      AppUtils.log("✅ Form validation passed");
      final currentContext = context;

      try {
        AppUtils.log("🚀 Calling AuthCtrl.find.login...");
        final result = await AuthCtrl.find
            .login(emailController.text, passwordController.text)
            .applyLoader;

        AppUtils.log("📦 Login result received: ${result?.isSuccess}");

        if (result != null && result.isSuccess) {
          AppUtils.log("✅ Login SUCCESS - navigating to HomeScreen");
          AppUtils.log("📦 User data: ${result.data}");
          //  _getFcmToken();
          if (mounted) {
            AppUtils.log("🏠 Pushing HomeScreen");
            currentContext.pushAndClearNavigator(HomeScreen());
          } else {
            AppUtils.log("⚠️ Widget not mounted, skipping navigation");
          }
        } else {
          AppUtils.log("❌ Login FAILED");
          AppUtils.log("❌ Result isSuccess: ${result?.isSuccess}");
          AppUtils.log("❌ Result error: ${result?.error}");
          ...
          AppUtils.log("❌ Showing error toast: $errorMessage");
          AppUtils.toastError(errorMessage);
        }
      } catch (e) {
        AppUtils.log("❌ Exception in _submitForm: $e");
